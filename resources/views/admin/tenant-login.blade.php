...
<title>{{ $tenantName }} — Administração</title>
...
<div class="login-wrapper">
  <aside class="brand-panel">
    ...
    <div class="brand-content">
      <h1 class="brand-title">Gerencie sua barbearia com simplicidade e estilo.</h1>
      <p class="brand-subtitle">Junte-se a milhares de barbearias que usam o Barber Control Pro para organizar agendamentos, equipe e finanças em um só lugar.</p>
      <div class="feature-list">
        <div class="feature-item"><div class="feature-ic"><svg class="icon icon-sm"><use href="#i-calendar"/></svg></div><span>Agenda inteligente e automatizada</span></div>
        <div class="feature-item"><div class="feature-ic"><svg class="icon icon-sm"><use href="#i-wallet"/></svg></div><span>Controle financeiro e comissões</span></div>
        <div class="feature-item"><div class="feature-ic"><svg class="icon icon-sm"><use href="#i-chart"/></svg></div><span>Relatórios detalhados em tempo real</span></div>
      </div>
    </div>
  ...
  <main class="form-panel">
    ...
    <div class="login-card">
      ...
      <form method="POST" action="{{ route('tenant.login.store', $barbearia->slug) }}">
        @csrf
        <div class="form-group">
          <label class="form-label">E-mail</label>
          <div class="input-group">
            <span class="input-icon"><svg class="icon icon-sm"><use href="#i-mail"/></svg></span>
            <input type="email" name="email" class="form-input" placeholder="seu@email.com" value="{{ old('email') }}" required autofocus>
          </div>
        </div>
        <div class="form-group">
          <label class="form-label">Senha</label>
          <div class="input-group">
            <span class="input-icon"><svg class="icon icon-sm"><use href="#i-lock"/></svg></span>
            <input type="password" name="password" class="form-input" placeholder="Sua senha secreta" required>
          </div>
        </div>
        <div class="form-options">
          <label class="checkbox-wrap">
            <input type="checkbox" name="remember" {{ old('remember') ? 'checked' : '' }}>
            <span class="custom-checkbox"><svg><use href="#i-check"/></svg></span>
      ...
      <div style="margin-top: 24px; text-align: center; border-top: 1px solid var(--border); padding-top: 20px;">
        <a href="{{ route('tenant.barbeiro.login', $barbearia->slug) }}" style="color: var(--text-muted); font-size: 13px; text-decoration: none; font-weight: 500; display: inline-flex; align-items: center; gap: 6px;">
